...
export class TransactionService {
   ...
   */
  async getRecentTransactions(limit: number = 20): Promise<TransactionWithDetails[]> {
    try {
      return await transactionRepository.getRecent(limit);
    } catch (error) {
      console.error('[TransactionService] Error getting recent transactions:', error);
  ...
  async getTransactionById(id: number): Promise<TransactionWithDetails> {
    try {
      ...
      throw error instanceof Error ? error : new Error('Failed to load transaction');
    }
  }
  ...
  async getTransactionsByAccount(accountId: number): Promise<TransactionWithDetails[]> {
    try {
      ...
      console.error('[TransactionService] Error getting transactions by account:', error);
   ...
   */
  async getTransactionsByCategory(categoryId: number): Promise<TransactionWithDetails[]> {
    try {
      return await transactionRepository.getByCategory(categoryId);
    } catch (error) {
      console.error('[TransactionService] Error getting transactions by category:', error);
  ...
  async updateTransaction(id: number, data: Partial<NewTransaction>): Promise<Transaction> {
    try {
      ...
      const originalDelta = original.type === 'income' ? original.amount : -original.amount;
   ...
   */
  private async updateAccountBalance(accountId: number, delta: number): Promise<void> {
