...
export function AccountsList() {
  const { accounts, isLoading, error, refresh, deleteAccount, totalBalance } = useAccounts();
  ...
  return (
    <View surface="page" style={styles.container}>
      <View surface="page" style={styles.header}>
        <View colorValue="transparent" style={styles.headerBalanceBlock}>
          <Text variant="bodySmall" color="muted">
            Total Balance
          </Text>
          <Text variant="heading1">{formatCurrency(totalBalance)}</Text>
        </View>

        <Button variant="primary" size="sm" label="+ Add" onPress={addSheet.open} />
      </View>
      ...
      {isLoading && accounts.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <AccountCard account={item} onDelete={() => handleDeleteAccount(item.id, item.name)} />
          )}
          ...
          ListEmptyComponent={
            <View colorValue="transparent" style={styles.emptyContainer}>
              ...
              <Button variant="primary" size="lg" label="Create Account" onPress={addSheet.open} />
            </View>
          }
