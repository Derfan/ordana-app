import { accountRepository, type Account, type NewAccount } from '@db/repositories';
...
export class AccountService {
  ...
  async getAccountById(id: number): Promise<Account> {
    try {
      ...
      console.error('[AccountService] Error getting account by id:', error);
      throw error instanceof Error ? error : new Error('Failed to load account');
    }
  }
  ...
  async updateAccount(id: number, data: Partial<NewAccount>): Promise<Account> {
    try {
      ...
      if (data.name) {
        const existingAccounts = await accountRepository.getAll();
        const duplicate = existingAccounts.find(
          (acc) => acc.id !== id && acc.name.toLowerCase() === data.name!.toLowerCase()
        );
  ...
  private validateAccountData(data: NewAccount, isPartial: boolean = false): void {
