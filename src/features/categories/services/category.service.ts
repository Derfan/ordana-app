...
export class CategoryService {
  ...
  async getCategoriesByType(type: CategoryType): Promise<Category[]> {
    try {
      ...
      console.error('[CategoryService] Error getting categories by type:', error);
      throw new Error('Failed to load categories');
    }
  }
  ...
  async getCategoryById(id: number): Promise<Category> {
    try {
      ...
      throw error instanceof Error ? error : new Error('Failed to load category');
    }
  }
  ...
  async createCategory(data: NewCategory): Promise<Category> {
    try {
      ...
      const existingCategories = await categoryRepository.getByType(data.type);
   ...
   */
  async updateCategory(id: number, data: Partial<NewCategory>): Promise<Category> {
    try {
      ...
      if (data.name) {
        ...
        const existingCategories = await categoryRepository.getByType(type);
        const duplicate = existingCategories.find(
   ...
   */
  private validateCategoryData(data: NewCategory, isPartial: boolean = false): void {
