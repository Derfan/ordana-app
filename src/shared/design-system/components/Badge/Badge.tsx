...
export function Badge({ label, variant = 'neutral', size = 'md', dotColor }: BadgeProps) {
  const theme = useTheme();

  const colorMap: Record<BadgeVariant, ColorPair> = {
    income: {
      background: theme.colors.bg.positiveSubtle,
      text: theme.colors.fg.positive,
    },
    expense: {
      background: theme.colors.bg.negativeSubtle,
      text: theme.colors.fg.negative,
    },
    neutral: {
      background: theme.colors.bg.muted,
      text: theme.colors.fg.muted,
    },
    brand: {
      background: theme.colors.bg.brandSubtle,
      text: theme.colors.accent.brand,
    },
    danger: {
      background: theme.colors.bg.dangerSubtle,
      text: theme.colors.fg.danger,
    },
  };
  ...
  return (
    <View style={[styles.base, padding, { backgroundColor: background }]}>
      {dotColor != null && <View style={[styles.dot, { backgroundColor: dotColor }]} />}

      <Text variant={typographyVariant} colorValue={text} numberOfLines={1}>
        {label}
      </Text>
    </View>
  );
