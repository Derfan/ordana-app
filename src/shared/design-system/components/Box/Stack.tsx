...
export interface StackProps extends Omit<BoxProps, 'direction' | 'gap' | 'align' | 'justify'> {
  /**
 ...
 * </HStack>
 */
export function Stack({ direction = 'vertical', gap, align, justify, ...boxProps }: StackProps) {
  const flexDirection: ViewStyle['flexDirection'] = direction === 'horizontal' ? 'row' : 'column';

  const resolvedAlign = align !== undefined ? ALIGN_MAP[align] : undefined;
  const resolvedJustify = justify !== undefined ? JUSTIFY_MAP[justify] : undefined;
